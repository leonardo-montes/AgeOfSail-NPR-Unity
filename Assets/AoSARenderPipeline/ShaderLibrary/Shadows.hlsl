#ifndef AOSA_SHADOWS_INCLUDED
#define AOSA_SHADOWS_INCLUDED
...
float GetDirectionalShadowAttenuation (DirectionalShadowData directional, ShadowData global, float3 normal, float3 position)
{
	...
	{
		shadow = GetCascadedShadow(directional, global, normal, position);
	}
	return shadow;
}
...
float GetOtherShadow (OtherShadowData other, ShadowData global, float3 normal, float3 position)
{
	...
	float3 surfaceToLight = other.lightPositionWS - position;
	float distanceToLightPlane = dot(surfaceToLight, lightPlane);
	float3 normalBias = normal * (distanceToLightPlane * tileData.w);
	float4 positionSTS = mul(_OtherShadowMatrices[tileIndex], float4(position + normalBias, 1.0));
	return FilterOtherShadow(positionSTS.xyz / positionSTS.w, tileData.xyz);
}

float GetOtherShadowAttenuation (OtherShadowData other, ShadowData global, float3 normal, float3 position)
{
	#if !defined(_RECEIVE_SHADOWS)
		return 1.0;
	#endif
	...
	{
		shadow = GetOtherShadow(other, global, normal, position);
	}
	return shadow;
}
