...
public class CustomShaderGUI : ShaderGUI
{
	...
	bool showPresets, showEdgeBreakupWarpPass, showBaseSettings, detailMapSettings, shadowSettings, transparencySettings, normalMapSettings;
	...
	public override void OnGUI(MaterialEditor materialEditor, MaterialProperty[] properties)
	{
		...
		if (isDefault)
		{
			bool useAgeOfSailPipeline = RenderPipelineManager.currentPipeline.GetType() == typeof(CustomRenderPipeline) && ((CustomRenderPipeline)RenderPipelineManager.currentPipeline).ageOfSailPipelineSettings.usePipeline;
			showBaseSettings = EditorGUILayout.Foldout(showBaseSettings, useAgeOfSailPipeline ? "Base settings (Age of Sail Render Pipeline)" : "Base settings (PBR)", true, EditorStyles.foldoutHeader);
			if (showBaseSettings)
			{
				++EditorGUI.indentLevel;
				// Default PBR pipeline
				if (!useAgeOfSailPipeline)
				{
					...
					if (FindProperty("_MaskMapToggle", properties, false) != null)
					{
						...
						prop0 = FindProperty("_Metallic", properties);
						editor.RangeProperty(prop0, prop0.displayName);
						
						prop0 = FindProperty("_Occlusion", properties);
						editor.RangeProperty(prop0, prop0.displayName);
						
						prop0 = FindProperty("_Smoothness", properties);
						editor.RangeProperty(prop0, prop0.displayName);
						
						prop0 = FindProperty("_Fresnel", properties);
						editor.RangeProperty(prop0, prop0.displayName);
						
						// Normal map
						EditorGUILayout.Space();
						enabled = FoldoutWithToggleKeyword(ref normalMapSettings, "_NormalMapToggle", "_NORMAL_MAP");
						if (normalMapSettings)
						{
							++EditorGUI.indentLevel;
							prop0 = FindProperty("_NormalMap", properties);
							editor.TexturePropertySingleLine(new GUIContent(prop0.displayName), prop0);
							
							prop0 = FindProperty("_NormalScale", properties);
							editor.RangeProperty(prop0, prop0.displayName);
							--EditorGUI.indentLevel;
							EditorGUILayout.Space();
						}
						...
						// Emission
						prop0 = FindProperty("_EmissionMap", properties);
						prop1 = FindProperty("_EmissionColor", properties);
						editor.TexturePropertyWithHDRColor(new GUIContent(prop0.displayName), prop0, prop1, true);

						// Detail map
						enabled = FoldoutWithToggleKeyword(ref detailMapSettings, "_DetailMapToggle", "_DETAIL_MAP");
						if (detailMapSettings)
						{
							++EditorGUI.indentLevel;
							prop0 = FindProperty("_DetailMap", properties);
							editor.TextureProperty(prop0, prop0.displayName);
							
							prop0 = FindProperty("_DetailNormalMap", properties);
							editor.TexturePropertySingleLine(new GUIContent(prop0.displayName), prop0);
							
							prop0 = FindProperty("_DetailAlbedo", properties);
							editor.RangeProperty(prop0, prop0.displayName);

							prop0 = FindProperty("_DetailSmoothness", properties);
							editor.RangeProperty(prop0, prop0.displayName);

							prop0 = FindProperty("_DetailNormalScale", properties);
							editor.RangeProperty(prop0, prop0.displayName);
							--EditorGUI.indentLevel;
						}
						GUI.enabled = enabled;
				...
				else
				{
					...
					if (FindProperty("_MaskMapToggle", properties, false) != null)
					{
						...
						// Normal map
						EditorGUILayout.Space();
						enabled = FoldoutWithToggleKeyword(ref normalMapSettings, "_NormalMapToggle", "_NORMAL_MAP", false);
						if (normalMapSettings)
						{
							++EditorGUI.indentLevel;
							prop0 = FindProperty("_NormalMap", properties);
						...
						// Detail map
						enabled = FoldoutWithToggleKeyword(ref detailMapSettings, "_DetailMapToggle", "_DETAIL_MAP", false);
						if (detailMapSettings)
						{
							++EditorGUI.indentLevel;
							prop0 = FindProperty("_DetailMap", properties);
		...
		{
			if (isDefault)
				enabled = FoldoutWithToggle(ref showEdgeBreakupWarpPass, "_EdgeBreakup");
			else
			{
				EditorGUILayout.LabelField("Edge Breakup Warp Pass", EditorStyles.boldLabel);
				showEdgeBreakupWarpPass = true;
				enabled = GUI.enabled;
			}
			if (showEdgeBreakupWarpPass)
			{
				if (isDefault)
					++EditorGUI.indentLevel;
				...
				prop0 = FindProperty("_EdgeBreakupDistanceFadeMultiplier", properties);
				editor.FloatProperty(prop0, prop0.displayName);
					
				prop0 = FindProperty("_EdgeBreakupWidthMultiplier", properties);
				editor.FloatProperty(prop0, prop0.displayName);
					
				prop0 = FindProperty("_EdgeBreakupSkew", properties);
				editor.FloatProperty(prop0, prop0.displayName);
				if (isDefault)
					--EditorGUI.indentLevel;
				EditorGUILayout.Space();
	...
	void SetEdgeBreakupWarpPass()
	{
		MaterialProperty edgeBreakup = FindProperty("_EdgeBreakup", properties, false);
		if (edgeBreakup == null || edgeBreakup.hasMixedValue)
		{
			return;
		}
		bool enabled = edgeBreakup.floatValue != 0.0f;
		foreach (Material m in materials)
		{
			m.SetShaderPassEnabled("EdgeBreakup", enabled);
		}
	}
}
