...
public class CustomMeshPostProcessor : AssetPostprocessor
{
    /*private static void OnPostprocessAllAssets(string[] importedAssets, string[] deletedAssets, string[] movedAssets, string[] movedFromAssetPaths, bool didDomainReload)
    {
        ...
        {
            if (AssetDatabase.GetMainAssetTypeAtPath(str) == typeof(GameObject))
            {
                Debug.Log("Reimported Asset: " + str);
                ProcessGameObject(AssetDatabase.LoadAssetAtPath<GameObject>(str));
            }
        }

        for (int i = 0; i < movedAssets.Length; i++)
        {
            if (AssetDatabase.GetMainAssetTypeAtPath(movedAssets[i]) == typeof(GameObject))
            {
                Debug.Log("Moved Asset: " + movedAssets[i] + " from: " + movedFromAssetPaths[i]);
                ProcessGameObject(AssetDatabase.LoadAssetAtPath<GameObject>(movedAssets[i])); 
            }
        }
    }*/
    ...
    {
        ProcessGameObject(gameObject, context);
    }
    ...
    private static void ProcessGameObject (GameObject gameObject, UnityEditor.AssetImporters.AssetImportContext context)
    {
        ...
        for (int i = 0; i < meshFilters.Length; ++i)
            GenerateInflationNormals(ref vertices, ref uv0, ref triangles, ref referenceVertexIdMap, ref edges, ref inflationNormals, gameObject, meshFilters[i].sharedMesh, context);
    }
                                                 ...
                                                 UnityEditor.AssetImporters.AssetImportContext context)
    {
        ...
        // Get the edges
        if (!MeshInflateHelper.GetEdges(vertices, triangles, ref referenceVertexIdMap, ref edges))
        {
        ...
        MeshInflateHelper.MeshType meshType = MeshInflateHelper.TryGenerateClosedMeshFromOpenMesh(ref vertices, ref uv0, ref triangles, edges, referenceVertexIdMap);
        ...
        if (meshType == MeshInflateHelper.MeshType.Closed)
        {
            ...
            MeshInflateHelper.GenerateInflationNormals(vertices, triangles, edges, referenceVertexIdMap, ref inflationNormals, 1.0f);
        ...
        // Open or Semi-Open mesh: create a brand new mesh.
            ...
            CreateNewInflationMesh(ref vertices, ref uv0, ref triangles, ref referenceVertexIdMap, ref edges, ref inflationNormals, gameObject, mesh, context);
    }
